...
// NFT Gallery — NIK binding dilakukan oleh faskes, bukan self-service
import { useState, useEffect } from "react";
import QRCode from "react-qr-code";
import {
  getOwnedCertificates, getTokenMetadata, getRecordByTokenId,
  claimCertificate, getNikHashByWallet, formatTimestamp,
} from "../utils/contract";
...
export default function CertificateGallery({ session }) {
  ...
  async function loadAll() {
    setLoading(true);
    try {
      // Cek apakah NIK sudah di-bind oleh faskes
      const boundHash = await getNikHashByWallet(provider, address);
      setNikBound(!!boundHash && boundHash !== ZERO_HASH);

      // Load owned NFTs berdasarkan wallet address
      ...
      const mine   = stored.filter(c => c.patientAddress?.toLowerCase() === address.toLowerCase());
  ...
  if (loading) {
    return <div style={s.center}><span style={s.spinner}/>&nbsp; Memuat sertifikat...</div>;
  }
      ...
      {certs.length === 0 ? (
        <div style={s.empty}>
          ...
          <div style={{color:"#64748b",fontSize:"14px",lineHeight:"1.7"}}>
            Sertifikat akan muncul setelah faskes mencatat vaksinasi.<br/>
            Pastikan faskes menggunakan wallet address kamu:<br/>
            <code style={s.addressCode}>{address}</code>
          </div>
        </div>
      ) : (
        <div style={s.grid}>
          ...
          <div style={s.modal} onClick={e => e.stopPropagation()}>
            ...
            <div style={s.modalBody}>
              ...
              <div style={s.certInfo}>
                <h3 style={s.infoTitle}>{selectedCert.certId || selectedCert.name}</h3>

                {/* QR Code — Error Correction H (30% damage recovery) */}
                <div style={s.qrSection}>
                  ...
                  <div style={s.qrText}>
                    <strong style={{color:"#f1f5f9",fontSize:"14px"}}>QR Verifikasi</strong>
                    <span style={{color:"#64748b",fontSize:"12px",lineHeight:"1.5"}}>
                      Scan di tab Verifikasi untuk cek keaslian sertifikat
                    </span>
                    <div style={s.tokenIdBox}>
                      <span style={s.tokenIdLabel}>Token ID</span>
                      <span style={s.tokenIdVal}>#{selectedCert.id}</span>
                    </div>
                  </div>
                </div>
...
const s = {
  ...
  nikBound:     {display:"flex",flexDirection:"column",gap:"6px",padding:"14px 18px",borderRadius:"10px",background:"rgba(16,185,129,0.08)",border:"1px solid rgba(16,185,129,0.25)",color:"#6ee7b7",fontSize:"14px"},
  nikUnbound:   {display:"flex",flexDirection:"column",gap:"6px",padding:"14px 18px",borderRadius:"10px",background:"rgba(59,130,246,0.06)",border:"1px solid rgba(59,130,246,0.2)",color:"#93c5fd",fontSize:"14px"},
  nikSub:       {fontSize:"12px",color:"#64748b",fontWeight:400,marginTop:"2px"},
  ...
  empty:        {padding:"48px 20px",borderRadius:"12px",background:"#111827",border:"1px solid #1e293b",textAlign:"center"},
  addressCode:  {display:"block",marginTop:"10px",fontFamily:"'JetBrains Mono',monospace",fontSize:"11px",color:"#60a5fa",background:"#0f172a",padding:"6px 10px",borderRadius:"6px",wordBreak:"break-all"},
  grid:         {display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:"20px"},
  card:         {borderRadius:"12px",background:"#111827",border:"1px solid #1e293b",overflow:"hidden",cursor:"pointer",transition:"transform 0.15s, box-shadow 0.15s"},
  ...
  certIdSmall:  {fontSize:"11px",fontFamily:"'JetBrains Mono',monospace",color:"#60a5fa",fontWeight:600},
  ...
  modal:        {maxWidth:"820px",width:"100%",background:"#0f172a",borderRadius:"20px",border:"1px solid #1e293b",position:"relative",maxHeight:"90vh",overflowY:"auto"},
  ...
  certPreview:  {flex:"1",minWidth:"180px",maxWidth:"240px"},
  ...
  infoTitle:    {fontSize:"20px",fontWeight:800,color:"#10b981",fontFamily:"'JetBrains Mono',monospace"},
  qrSection:    {display:"flex",alignItems:"center",gap:"20px",padding:"16px",background:"#1a2332",borderRadius:"12px",border:"1px solid #1e293b"},
  qrWrapper:    {padding:"10px",background:"white",borderRadius:"10px",flexShrink:0},
  qrText:       {display:"flex",flexDirection:"column",gap:"8px"},
  tokenIdBox:   {padding:"8px 12px",borderRadius:"6px",background:"rgba(96,165,250,0.1)",border:"1px solid rgba(96,165,250,0.2)"},
  tokenIdLabel: {display:"block",fontSize:"10px",color:"#64748b",textTransform:"uppercase",marginBottom:"2px"},
  tokenIdVal:   {fontFamily:"'JetBrains Mono',monospace",fontSize:"13px",color:"#60a5fa",fontWeight:700},
